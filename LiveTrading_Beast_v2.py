...
LOSING_SYMBOLS = ['ADAUSDT', 'XRPUSDT', 'DOGEUSDT']
...
# ===== NO SL MODE =====
# TP: +4% (exit and take profit)
# SL: NONE (let positions recover)
...
def place_order(symbol, side):
    try:
        ...
        # Mark as entered to prevent duplicates
...
# ================== MAIN BEAST LOOP WITH ANTI-DEGEN ==================
if __name__ == "__main__":
    logging.info("🔥 BEAST MODE v2 ACTIVATED — ANTI-DEGEN UPGRADED")
    logging.info(f"Trading only WINNERS: {WINNING_SYMBOLS}")
    logging.info(f"Blacklisted LOSERS: {LOSING_SYMBOLS}")
    logging.info(f"TP: REAL +10% | PARTIAL: +4% (80% close, 20% runner)")
    logging.info(f"Entry Score: 5+ points (very selective)")
    logging.info("✅ NEWS FILTER: FOMC, Fed, CPI, PPI, SEC, hacks, exploits")
    logging.info("✅ DAILY LIMITS: +8% profit target, -3% loss limit, max 10 trades")
    logging.info("✅ SLIPPAGE KILLER: Limit orders + 0.5% buffer + market fallback")
    ...
    while True:
        try:
            ...
            # Clear entered symbols if they're no longer open (position closed)
