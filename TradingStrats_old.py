...
# DEPRECATED: Old case statements removed for Python 3.9 compatibility
# All functionality preserved in the unified if/elif/else block above
# Lines 732-881 removed to fix Python 3.9 syntax errors
...
def horlah_40percent_master(
    ...
    # ADD THIS AT THE VERY BOTTOM OF TradingStrats.py (after all other strategies)
...
def horlah_40percent_master(
    ...
    # ─── SAFETY FIRST: DAILY LIMITS ─────────────────────────────────────
    daily_return = (current_balance - starting_balance) / starting_balance
    if daily_return >= 0.40:    # +40% today? WE CHILLIN' LIKE CHAMPIONS
        logger.info(f" {symbol} | DAILY 40% TARGET HIT! Bot takes the rest of the day off!")
        return -99  # No new trades today
    if daily_return <= -0.10:   # -10%? Emergency shutdown
        logger.critical(f" {symbol} | MAX DRAWDOWN HIT! Bot stopped to protect capital.")
        return -99

    # ─── ACCUMULATION BREAKOUT LONG (15min logic) ───────────────────────
    if len(Close) < 50:
        return Trade_Direction

    # Bollinger Bands squeeze detection (accumulation)
    window = 20
    bb_std = pd.Series(Close).rolling(window).std()
    bb_mid = pd.Series(Close).rolling(window).mean()
    bb_upper = bb_mid + 2 * bb_std
    bb_lower = bb_mid - 2 * bb_std

    squeeze = (bb_upper - bb_lower).iloc[-10:]  # Last 10 candles
    current_squeeze = squeeze.iloc[-1]
    prev_squeeze = squeeze.iloc[-2]

    # Squeeze + expansion + volume spike + breakout above range
    if (current_squeeze > prev_squeeze * 1.1 and 
        Close[current_index] > High[current_index-5:current_index].max() and
        Volume[current_index] > np.mean(Volume[-20:]) * 2 and
        Close[current_index] > bb_upper.iloc[-1]):
        
        logger.info(f" {symbol} | ACCUMULATION BREAKOUT LONG!")
        Trade_Direction = 1
        return Trade_Direction

    # ─── SHORT SCALP: 3-SEC SPIKE REVERSAL (fakeout trap) ─────────────────
    # We simulate "3-second" data using 1-minute candles + big wick detection
    lookback = 5
    recent_high = max(High[current_index-lookback:current_index+1])
    recent_low = min(Low[current_index-lookback:current_index+1])
    spike_size = (recent_high - recent_low) / recent_low

    # Detect massive green candle (spike up) followed by strong red rejection
    if (spike_size > 0.008 and  # >0.8% spike in 5 mins
        Close[current_index-1] > Open[current_index-1] and
        (High[current_index-1] - Close[current_index-1]) < (Close[current_index-1] - Open[current_index-1]) * 0.2 and  # small upper wick
        Close[current_index] < Open[current_index] and
        Close[current_index] < Low[current_index-1] and
        Volume[current_index] > Volume[current_index-1] * 1.5):

        logger.info(f" {symbol} | 3-SEC SPIKE REVERSE SHORT!")
        Trade_Direction = 0
        return Trade_Direction
...
# =====================================================
# HORLAH x GROK — 40% DAILY SCALPER (2025 CHAMPION)
# =====================================================

def horlah_40percent_master(Trade_Direction, Close, High, Low, Open, Volume, current_index, symbol, 
                          daily_pnl=0.0, starting_balance=1000.0, current_balance=1000.0):
    from Logger import logger
    import pandas as pd
    import numpy as np

    # Daily 40% profit → bot sleeps like a king
    daily_return = (current_balance - starting_balance) / starting_balance
    if daily_return >= 0.40:
        logger.info(f" {symbol} │ +40% TODAY — BOT IS TAKING THE DAY OFF LIKE A BOSS")
        return -99
    if daily_return <= -0.10:
        logger.critical(f" {symbol} │ MAX DRAWDOWN — SHUTTING DOWN TO PROTECT CAPITAL")
        return -99
    ...
    # LONG: Accumulation breakout after Bollinger squeeze
    bb_mid = Close.rolling(20).mean()
    bb_std = Close.rolling(20).std()
    bandwidth = (bb_mid + 2*bb_std) - (bb_mid - 2*bb_std)
    
    if (bandwidth.iloc[-1] > bandwidth.iloc[-2:-10].mean() * 1.3 and
        Close.iloc[-1] > High.iloc[-7:-1].max() and
        Volume.iloc[-1] > Volume.iloc[-15:].mean() * 2.5 and
        Close.iloc[-1] > bb_mid.iloc[-1] + bb_std.iloc[-1]):
        
        logger.info(f" {symbol} │ ACCUMULATION BREAKOUT — LONG SIGNAL!")
        return 1

    # SHORT: Violent spike + instant reversal (fakeout trap)
    spike_range = High.iloc[-6:-1].max() - Low.iloc[-6:-1].min()
    spike_percent = spike_range / Low.iloc[-6:-1].min()
    
    if (spike_percent > 0.009 and
        Close.iloc[-2] > Open.iloc[-2] and
        Close.iloc[-1] < Open.iloc[-1] and
        Close.iloc[-1] < Low.iloc[-2] and
        Volume.iloc[-1] > Volume.iloc[-2] * 1.7):
        
        logger.info(f" {symbol} │ SPIKE & REVERSE — SHORT SIGNAL!")
        return 0
